...
On souhaite installer la version 8.2 de PHP, mais les catalogues logiciels officiels de Ubuntu ne fournissent pas encore cette version à l'heure de la rédaction de cette procédure (juin 2024). Il nous faut donc ajouter un catalogue capable de nous fournir PHP 8.2 à la configuration de `apt`.

Pour cela, on installe d'abord l'ensemble d'utilitaires `software-properties-common` qui va nous permettre de gérer simplement les catalogues logiciels utilisés par `apt`. Ensuite, on ajoute le catalogue `ondrej/php`, on met à jour la liste des logiciels disponibles et on lance un petit `apt upgrade` pour mettre à jour les logiciels déjà installé pour lesquels il existerait désormais une version plus récente de disponible. Toutes ces commandes nécessitent d'être exécutées avec les privilèges administrateur, d'où l'utilisation de `sudo`.

```bash
sudo apt install -y php8.2-common php8.2-cli php8.2-mysql php8.2-intl php8.2-xml php8.2-zip libapache2-mod-php8.2
```

Enfin, encore une fois, on installe des packages. Cette fois-ci, il s'agit de PHP en lui-même, c'est-à-dire l'interpréteur du code ainsi que certaines de ses extensions et modules :
- `php8.2-common` est l'interpréteur PHP (version 8.2) en soi, il permet d'exécuter du code PHP
- `php8.2-cli` fourni l'interface en ligne de commande de PHP, te permettant ainsi d'interagir avec l'interpréteur directement depuis le terminal
- `php8.2-mysql` est une extension de PHP qui fournit des drivers et des fonctions permettant au code d'interagir avec des bases de données MySQL
- `php8.2-xml` est une extension de PHP qui introduit des fonctionnalités de manipulation de données au format XML et ses dérivés (HTML, etc.), de telles structures sont utilisées par Symfony
- `php8.2-intl` est une extension de PHP dédiée à l'internationalisation et la gestion de traductions, elle est requise par Symfony
- `php8.2-zip` est une extension de PHP qui lui permet de manipuler des archives ZIP, elle est notamment utilisée par Composer pour optimiser le téléchargement des dépendances
- `libapache2-mod-php8.2` est un module pour Apache qui lui permet d'interagir avec l'interpréteur PHP, sans lui il serait impossible de passer les informations de la requête HTTP à PHP et de comprendre les réponses générées par ton code
